...
func (r Registrator) Registration(machine *statemachine.StateMachine, w window.Window, config onmyoji.Config, imgMap map[string]onmyoji.ImgInfo) error {
	...
	onmyoji.Registration(machine, newAttackDetector(w, imgMap[string(tasks.BreakerAttack)], config))
	onmyoji.Registration(machine, newRewardDetector(w, rewardList))
	onmyoji.Registration(machine, newBreakerFailDetector(w, imgMap[string(tasks.BreakerFail)]))
	onmyoji.Registration(machine, newBreakerWinDetector(w, imgMap[string(tasks.BreakerWin)]))
...
func (r Registrator) LoadImageTemplates() (map[string]onmyoji.ImgInfo, error) {
	...
	requiredImages := []func() (string, bool){
		func() (string, bool) {
			_, exists := imgMap[string(tasks.BreakerAttack)]
			return string(tasks.BreakerAttack), exists
		},
		func() (string, bool) {
		...
		func() (string, bool) {
			_, exists := imgMap[string(tasks.BreakerFail)]
			return string(tasks.BreakerFail), exists
		},
		func() (string, bool) {
			_, exists := imgMap[string(tasks.BreakerWin)]
			return string(tasks.BreakerWin), exists
		},
		func() (string, bool) {
			for imgName := range imgMap {
				if strings.HasPrefix(imgName, "breaker_player_") {
