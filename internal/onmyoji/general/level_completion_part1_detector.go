...
func newLevelCompletionDetector(window window.Window, info onmyoji.ImgInfo, config onmyoji.Config) *LevelCompletionDetector {
...
func (t *LevelCompletionDetector) Name() tasks.TaskType {
	return tasks.LevelCompletion
}
func (t *LevelCompletionDetector) Execute(controller statemachine.TaskController) error {
	...
	controller.Next(tasks.LevelCompletionPart2) // 切换到任务完成状态
	return nil
}
