...
func (t *MonsterDetector) Name() tasks.TaskType {
	return tasks.XunGuai
}
...
func (t *MonsterDetector) Execute(controller statemachine.TaskController) error {
	...
	boss, err := controller.GetAttribute(tasks.BossState)
	...
	if hasBoss {
		...
		controller.Next(tasks.BaoXiang)    // 切换到宝箱任务
		return nil
	}
	if t.Count >= t.conf.XunguaiFindThreshold { // 如果执行次数超过阈值
		...
		controller.SetAttribute(tasks.MoveCount, randomMoveCount) // 重置计数器
		controller.Next(tasks.MoveTask)
		t.Count = 0 // 重置计数器
	}
