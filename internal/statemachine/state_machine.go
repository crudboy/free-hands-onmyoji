...
type TaskController interface {
	Next(key tasks.TaskType) error                              // 切换到指定名称的任务
	NextIndex()                                                 // 按索引切换到下一个任务
	GetAttribute(name tasks.TaskState) (interface{}, error)     // 获取任务属性，用于任务间信息传递
	SetAttribute(name tasks.TaskState, value interface{}) error // 设置任务属性，用于任务间信息传递
...
type StateMachine struct {
	tasks      []NamedTask
	current    int
	attributes map[tasks.TaskState]interface{} // 用于存储任务间共享的信息
}
...
func NewStateMachine() *StateMachine {
	...
	return &StateMachine{
		tasks:      []NamedTask{},
		current:    0,
		attributes: make(map[tasks.TaskState]interface{}), // 初始化属性映射
	}
}

// Next 根据任务名称切换到指定任务
func (sm *StateMachine) Next(key tasks.TaskType) error {
...
func (sm *StateMachine) ClearAttributes() {
	if sm.attributes != nil {
		sm.attributes = make(map[tasks.TaskState]interface{})
	}

}

// GetTaskByName 根据名称获取任务
func (sm *StateMachine) GetTaskByName(name tasks.TaskType) (NamedTask, error) {
...
func (sm *StateMachine) GetAttribute(name tasks.TaskState) (interface{}, error) {
	if sm.attributes == nil {
...
func (sm *StateMachine) SetAttribute(name tasks.TaskState, value interface{}) error {
	if sm.attributes == nil {
		sm.attributes = make(map[tasks.TaskState]interface{})
	}
