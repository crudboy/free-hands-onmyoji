...
type TaskRegistrator interface {
	...
	Registration(*statemachine.StateMachine, window.Window, Config, map[string]ImgInfo) error

	// LoadImageTemplates 加载图片模板
	LoadImageTemplates() (map[string]ImgInfo, error)
}
type Registrator struct {
	machine    *statemachine.StateMachine // 状态机
	config     Config                     // 任务配置
	windowInfo window.Window
	imgMap     map[string]ImgInfo // 模板图片信息
